    ...
    public ResponseEntity<ReqRes> updateArtist(@PathVariable Integer id, @RequestBody Artist updatedArtistData) {
        ...
        return artistService.getArtistById(id)
                .map(existingArtist -> {
                    ...
                    if (updatedArtistData.getProfile_image_url() != null && !updatedArtistData.getProfile_image_url().equals(existingArtist.getProfile_image_url())) {
                        existingArtist.setProfile_image_url(updatedArtistData.getProfile_image_url());
                    }
